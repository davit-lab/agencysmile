...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      const [content, teamList, leadsList, posts] = await Promise.all([
      ...
      setTeam(teamList);
      setLeads(leadsList);
      setBlogPosts(posts.length > 0 ? posts : Object.entries(initialTranslations.ka.blogPage.posts).map(([id, p]: any) => ({
        id,
